...
- **InputInterceptor**: Unity MonoBehaviour that handles accessibility hotkeys (F1, C, T, H, K, L, U, R, V, plus Ctrl+G/H/R). Routes input by mode: help browsing first, then Ctrl combos, then targeting, then floor review (open or entry via plain Up arrow in battle; unclaimed keys fall through), then plain hotkeys.
...
- **MapNavigator**: Virtual map cursor for the map screen (review-only, doesn't move real selection). Ctrl+Up/Down moves between rings (distances), Ctrl+Left/Right steps through a ring's stops. Stops come from the live map UI first: each ring's `MapSection.mapNodesByBranch` holds exactly the `MapNodeUI`s the game placed (UI slot caps applied, Hellforged pact nodes extracted to the `NoBranch = -1` group = reachable from either path), so the review lists what a sighted player sees. Fallback when the section isn't loaded: `RunState`/`NodeState` via the game's static `MapSection.GetMapNodeDataForBranch(distance, branch, saveManager)` so node `Location`s match the game's `CanBeTriggered`/`HasBeenVisited` indexing (per-type index: merchants, events, then rewards); there, nodes appearing in both branch lists are the same `MapNodeData` asset (reference equality) = shared "both paths" stops, and `GetRequiredDlc() == Hellforged` nodes in either list are treated as both-paths. Battle per ring via `SaveManager.GetScenarioData(distance)`.
...
- **FloorReviewSystem**: Virtual review cursor over the floors (plain Up arrow in battle opens it; review-only, never moves the real game selection — MT1 units are not clickable actors, hover tooltips are the only in-game inspection). Up/Down move between floors (room index 0=bottom → 3=pyre), Left/Right step through the floor's units in spatial order (your units back-to-front, then enemies front-to-back; rank 1 = front of its team), Enter reads full details, Escape closes (same-frame `ClosedThisFrame` latch). Game state is re-read on every keypress so the lineup stays current; focus details (keywords, intents, floor corruption/enchantments) feed the UI/Creature buffers via `FocusBuffers.SetReviewFocus`. Auto-closes on battle exit/win/loss, when either targeting system starts, and when the screen changes away from battle. The Up claim is gated on `IsBattleScreenFrontmost()` (per-frame cached): combat must be active (`IsInBattle`) AND `ScreenName.Game` must be active AND `GameScreen.IsInteractable()` true — the game's own focus notion: `ScreenManager.UpdateInteractable` walks the active screen stack top-down and only screens above the first `DoesBlockInteractions()` screen stay interactable. So ANY covering screen (pile views, Dialog, Settings/pause, Reward, Draft, run summary, even Tab-focusing the HUD) releases the arrows without being known by name. Two failed designs to not repeat: a denylist of overlay ScreenNames missed `Reward` and broke arrow reading on the post-battle reward screen; "top entry of `activeScreensSorted` (ignoring Hud) == Game" never matched because the loading screen stays registered active forever (highest priority, `HandleBaseScreenChange` returns true, it only hides its visuals) — `DoesBlockInteractions` is how the game itself excludes hidden-but-active screens. `ScreenStateTracker` can't do any of this because it only sees screens opening, not closing; it remains the fallback, kept honest mid-battle by `DeckScreenPatch.ClosePostfix` (restores `GameScreen.Battle` when a pile view closes during battle). The `IsInBattle` half of the gate is what shuts the review off after a battle ends: the post-battle **VictoryUI** (score + reward summary + Collect button) is a component of `GameScreen` itself, NOT a separate screen, so `ScreenName.Game` stays active and interactable while it shows — without `IsInBattle` going false, the Up arrow would wrongly open floor review on the reward summary. `BattleVictoryPatch` resets it by patching `CombatManager.StopCombat(bool combatWon)` (an `IEnumerator`; Harmony patches the kickoff, so the postfix fires the moment combat is told to stop, before VictoryUI shows). The older `EndCombat`/`OnCombatComplete` names never existed on `CombatManager`, so that patch was a silent no-op and `IsInBattle` never reset — the bug this fixed. `VictoryUIPatch` (postfix on `VictoryUI.Show`) reads the rewards being collected and marks the screen state `Rewards`.
...
Priority-based context-sensitive help. `HelpSystem` selects the highest-priority active `IHelpContext`. 20 contexts from `GlobalHelp` (priority 0) through `DialogHelp` (priority 110). Each context's `IsActive()` checks `ScreenStateTracker.CurrentScreen` (or mod state, e.g. `FloorReviewHelp` checks `FloorReview.IsActive`).
...
`MonsterTrainAccessibility.cs` is the BepInEx plugin entry. `Awake()` initializes all systems, `ApplyPatches()` registers ~57 Harmony patches, `CreateHandlers()` creates persistent MonoBehaviour GameObjects, `RegisterHelpContexts()` registers all 20 help contexts.
...
| R | Read ember |
| Up arrow | Open floor review: virtual cursor over the floors. Up/Down = floors (bottom → pyre), Left/Right = units in position order (yours back-to-front, then enemies front-to-back; Left past the first unit returns to the floor overview), Enter = full details, Escape = close. Details feed the UI/Creature buffers |
...
- **TMP `.text` returns prefab placeholders**: The game's `SetTextSafe` extension calls `TMP_Text.SetText()`, which updates the rendered char buffer but NOT the `text` property — reading `.text` returns the prefab's design-time placeholder (e.g. leaderboard values reading "12345" while the screen shows "0"). Use `UITextHelper.GetRenderedTMPLabelText()` (tries `GetParsedText()`, falls back to `.text`) for any label the game fills at runtime. For read-the-whole-screen dumps pass `fallbackToTextProperty: false` (or use `GetRenderedTMPTextDirect()`): everything visible has already rendered, and the `.text` fallback would resurrect placeholders for labels rendered empty — this is how the S/T dumps and the game-over auto-read avoid speaking "Card text goes here." on the run summary. Note `GetParsedText()` is a frame stale for labels updated the same frame, so focused-item readers (which run right after game code sets text) keep using `.text`-based helpers.
